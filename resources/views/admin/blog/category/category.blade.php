...
<main class="content-wrapper">
    ...
    <!-- Mobile Card View -->
    <div id="mobile-blog-categories-view" class="mobile-view-container">
        <div class="mb-3">
            <input type="text" id="mobileSearchInput" class="form-control" placeholder="Search Blog Categories...">
        </div>
        <div id="mobile-blog-categories-container">
            <!-- Blog categories will be loaded here via JavaScript -->
        </div>
        <div id="mobile-loading" class="text-center py-4">
            <div class="spinner-border text-primary" role="status">
                <span class="visually-hidden">Loading...</span>
            </div>
            <p class="mt-2">Loading blog categories...</p>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        function initializeTable() {
            ...
            if (isMobile) {
                $('#mobile-blog-categories-view').show();
                $('#mobile-loading').show();
                $('#mobile-blog-categories-container').html('');
                $('.client-table').hide();
            } else {
                $('#mobile-blog-categories-view').hide();
                $('#mobile-loading').hide();
                $('#mobile-blog-categories-container').html('');
                $('.client-table').show();
            }
            ...
            var tableConfig = {
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.get-blog-categories-list') }}",
                    type: 'GET',
                    data: function(d) {
                ...
                tableConfig.drawCallback = function(settings) {
                    ...
                    if (data.length > 0) {
                        ...
                        data.each(function(category) {
                            html += createBlogCategoryCard(category);
                        });
                        $('#mobile-blog-categories-container').html(html);
                    } else {
                        $('#mobile-blog-categories-container').html('<div class="text-center py-4"><p>No blog categories found</p></div>');
                    }
        ...
        function createBlogCategoryCard(category) {
            return `
                <div class="blog-category-card">
                    <div class="blog-category-card-header">
                        <div class="blog-category-card-title">${category.name}</div>
                    </div>
                    <div class="blog-category-card-actions">
                        ${category.action}
                    </div>
                </div>
            `;
