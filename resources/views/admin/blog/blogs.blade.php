...
<main class="content-wrapper">
    ...
    <!-- Mobile Card View -->
    <div id="mobile-blogs-view" class="mobile-view-container">
        <div class="mb-3">
            <input type="text" id="mobileSearchInput" class="form-control" placeholder="Search Blogs...">
        </div>
        <div id="mobile-blogs-container">
...
<script>
    $(document).ready(function() {
        ...
        function initializeTable() {
            ...
            if (isInitialized && $.fn.DataTable.isDataTable('#blog-table')) {
            ...
            if (isMobile) {
                $('#mobile-blogs-view').show();
                $('#mobile-loading').show();
                $('#mobile-blogs-container').html('');
                $('.client-table').hide();
            } else {
                $('#mobile-blogs-view').hide();
                $('#mobile-loading').hide();
                $('#mobile-blogs-container').html('');
                $('.client-table').show();
            }
            ...
            var tableConfig = {
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.get-blogs-list') }}",
                    type: 'GET',
                    data: function(d) {
            ...
            if (isMobile) {
                tableConfig.drawCallback = function(settings) {
                    ...
                    if (data.length > 0) {
                        ...
                        data.each(function(blog) {
                            html += createBlogCard(blog);
                        });
                        $('#mobile-blogs-container').html(html);
                    } else {
                        $('#mobile-blogs-container').html('<div class="text-center py-4"><p>No blogs found</p></div>');
                    }
                ...
                tableConfig.initComplete = function(settings, json) {
                    ...
                    if (json && json.data && json.data.length > 0) {
                        var html = '';
                        json.data.forEach(function(blog) {
                            html += createBlogCard(blog);
                        });
                        $('#mobile-blogs-container').html(html);
            ...
            table = $('#blog-table').DataTable(tableConfig);
            isInitialized = true;
            ...
            return `
                <div class="blog-card">
                    <div class="blog-card-header">
                        <div class="blog-card-title">${blog.name}</div>
                    </div>
                    <div class="blog-card-body">
                        <div class="blog-card-field">
                            <span class="blog-card-label">Category</span>
                            <span class="blog-card-value">${blog.category || 'N/A'}</span>
                        </div>
                        <div class="blog-card-field">
                            <span class="blog-card-label">Author</span>
                            <span class="blog-card-value">${blog.author || 'N/A'}</span>
                        </div>
                    </div>
