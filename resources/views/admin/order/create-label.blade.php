                                ...
                                <div class="items-body">
                                    ...
                                    <div class="item-row">
                                        <div class="item-left">
                                            ...
                                            <div class="prod-info" style="min-width: 0 !important; width: 100% !important; box-sizing: border-box !important;">
                                                <div class="prod-name">{{ $bundleMeta['name'] ?? 'Bundle' }}</div>

                                                <div>
                                                    <select class="form-select form-select-sm bundleProductSelect" data-index="{{ $loop->index }}" style="max-width: 100% !important; width: 100% !important; box-sizing: border-box !important;">
                    ...
                    <div class="prod-info" style="min-width: 0 !important; width: 100% !important; box-sizing: border-box !important;">
                        ...
                        <div>
                            <select class="form-select form-select-sm bangleProductSelect" 
                                    data-index="{{ $loop->index }}" style="max-width: 100% !important; width: 100% !important; box-sizing: border-box !important;">
    ...
    @endif
                    ...
                    <div class="card mb-3 total-card">
                        ...
                        <div class="card-body">
                            ...
                            <div style="display: flex; flex-direction: column; align-items: stretch; gap: 12px; width: 100%; box-sizing: border-box;">
                                <div style="flex: 1; width: 100%; box-sizing: border-box;"></div>

                                <!-- visually matching input with unit pill on right -->
                                <div style="width: 100% !important; max-width: 100% !important; box-sizing: border-box !important;">
    ...
    <script>
        ...
        // If province_code is empty, try to derive from state name
        if (!provinceCode) {
            const stateName = document.querySelector('#state')?.value || '{{ $addr["state_province"] ?? $addr["state"] ?? "" }}';
            if (stateName) {
                // Try to get first 2 letters of state name as fallback
                provinceCode = stateName.substring(0, 2).toUpperCase();
                console.warn('Province code derived from state name:', stateName, '->', provinceCode);
            }
        }
        ...
        // Validate province_code is not empty
        if (!provinceCode || provinceCode.trim() === '') {
            console.error('Province code is required but empty!');
            alert('Province/State code is required. Please update the shipping address.');
     ...
     */
    function downloadShippingLabel(shipment, retryCount = 0) {
        if (!shipment) {
            console.error('No shipment data available');
            return;
        }
        ...
        if (labelUrl) {
            ...
            // Label not ready yet - offer to retry or check Stallion dashboard
            Swal.fire({
                icon: 'info',
                title: 'Label Processing',
                html: `
                    <p>The label PDF is being generated by Stallion Express.</p>
                    <p>Ship Code: <strong>${shipment.ship_code || 'N/A'}</strong></p>
                ...
                denyButtonText: 'Open Stallion Dashboard',
                cancelButtonText: 'Cancel'
            }).then((result) => {
                if (result.isConfirmed && retryCount < 5) {
                    // Retry after 3 seconds
                    Swal.fire({
                        title: 'Retrying...',
                        text: 'Checking if label is ready',
                        timer: 3000,
                        ...
                        // Fetch updated shipment data and retry
                        fetchAndDownloadLabel(shipment.ship_code, retryCount + 1);
                    });
                } else if (result.isDenied) {
                    // Open Stallion dashboard
                    window.open('https://ship.stallionexpress.ca/shipments', '_blank');
                }
            });
        }
    }
    ...
    /**
     * Fetch label from Stallion API and download
     */
    async function fetchAndDownloadLabel(shipCode, retryCount = 0) {
        try {
            const response = await fetch(`https://ship.stallionexpress.ca/api/v4/shipments/${shipCode}`, {
                headers: {
                    'Authorization': 'Bearer {{ env("STALLION_API_TOKEN") }}',
                    'Accept': 'application/json'
                }
            });
            ...
            if (response.ok) {
                ...
                if (data.shipment) {
                    downloadShippingLabel(data.shipment, retryCount);
                } else {
                    throw new Error('Invalid response from Stallion API');
                }
            } else {
                throw new Error('Failed to fetch label from Stallion API');
            }
        } catch (error) {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to fetch label. Please check Stallion Express dashboard.',
