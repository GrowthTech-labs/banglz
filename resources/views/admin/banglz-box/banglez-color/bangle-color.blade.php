...
<main class="content-wrapper">
    ...
    <div class="client-table pt-2">
        ...
        <table id="bangle-color-table" style="width: 100%">
            <thead>
                <tr>
    ...
    <!-- Mobile Card View -->
    <div id="mobile-bangle-colors-view" class="mobile-view-container">
        <div class="mb-3">
            <input type="text" id="mobileSearchInput" class="form-control" placeholder="Search Bangle Color...">
        </div>
        <div id="mobile-bangle-colors-container">
            <!-- Bangle colors will be loaded here via JavaScript -->
        </div>
        <div id="mobile-loading" class="text-center py-4">
            <div class="spinner-border text-primary" role="status">
                <span class="visually-hidden">Loading...</span>
            </div>
            <p class="mt-2">Loading bangle colors...</p>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        function initializeTable() {
            ...
            if (isInitialized && $.fn.DataTable.isDataTable('#bangle-color-table')) {
                console.log('Destroying existing table');
                table.destroy();
                // Rebuild table structure
                $('#bangle-color-table').html(`
                    <thead>
                        <tr>
                            <th>Image</th>
                            <th>Color</th>
            ...
            if (isMobile) {
                $('#mobile-bangle-colors-view').show();
                $('#mobile-loading').show();
                $('#mobile-bangle-colors-container').html('');
                $('.client-table').hide();
            } else {
                $('#mobile-bangle-colors-view').hide();
                $('#mobile-loading').hide();
                $('#mobile-bangle-colors-container').html('');
                $('.client-table').show();
            }
            ...
            var tableConfig = {
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.get-bangle-color-list') }}",
                    type: 'GET',
                    data: function(d) {
                ...
                columns: [
                    ...
                    { data: 'color_name', name: 'color_name', orderable: true },
            ...
            if (isMobile) {
                tableConfig.drawCallback = function(settings) {
                    ...
                    if (data.length > 0) {
                        var html = '';
                        data.each(function(bangleColor) {
                            html += createBangleColorCard(bangleColor);
                        });
                        $('#mobile-bangle-colors-container').html(html);
                    } else {
                        $('#mobile-bangle-colors-container').html('<div class="text-center py-4"><p>No bangle colors found</p></div>');
                    }
                ...
                tableConfig.initComplete = function(settings, json) {
                    ...
                    if (json && json.data && json.data.length > 0) {
                        var html = '';
                        json.data.forEach(function(bangleColor) {
                            html += createBangleColorCard(bangleColor);
                        });
                        $('#mobile-bangle-colors-container').html(html);
            ...
            table = $('#bangle-color-table').DataTable(tableConfig);
            isInitialized = true;
        ...
        function createBangleColorCard(bangleColor) {
            return `
                <div class="bangle-color-card">
                    <div class="bangle-color-card-header">
                        <div class="bangle-color-card-title">${bangleColor.color_name || 'N/A'}</div>
                    </div>
                    <div class="bangle-color-card-image">
                        ${bangleColor.image}
                    </div>
                    <div class="bangle-color-card-body">
                        <div class="bangle-color-card-field">
                            <span class="bangle-color-card-label">Size</span>
                            <span class="bangle-color-card-value">${bangleColor.size || 'N/A'}</span>
                        </div>
                    </div>
                    <div class="bangle-color-card-actions">
                        ${bangleColor.action}
                    </div>
                </div>
            `;
