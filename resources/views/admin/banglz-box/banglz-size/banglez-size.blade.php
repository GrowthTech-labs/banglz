...
<main class="content-wrapper">
    ...
    <!-- Mobile Card View -->
    <div id="mobile-bangle-sizes-view" class="mobile-view-container">
        <div class="mb-3">
            <input type="text" id="mobileSearchInput" class="form-control" placeholder="Search Bangle Size...">
        </div>
        <div id="mobile-bangle-sizes-container">
            <!-- Bangle sizes will be loaded here via JavaScript -->
        </div>
        <div id="mobile-loading" class="text-center py-4">
            <div class="spinner-border text-primary" role="status">
                <span class="visually-hidden">Loading...</span>
            </div>
            <p class="mt-2">Loading bangle sizes...</p>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        function initializeTable() {
            ...
            if (isInitialized && $.fn.DataTable.isDataTable('#bangle-size-table')) {
                console.log('Destroying existing table');
                table.destroy();
                // Rebuild table structure
                $('#bangle-size-table').html(`
                    <thead>
                        <tr>
                            <th>Size</th>
                            <th>Action</th>
            ...
            if (isMobile) {
                $('#mobile-bangle-sizes-view').show();
                $('#mobile-loading').show();
                $('#mobile-bangle-sizes-container').html('');
                $('.client-table').hide();
            } else {
                $('#mobile-bangle-sizes-view').hide();
                $('#mobile-loading').hide();
                $('#mobile-bangle-sizes-container').html('');
                $('.client-table').show();
            }
            ...
            var tableConfig = {
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.get-bangle-size-list') }}",
                    type: 'GET',
                    data: function(d) {
            ...
            if (isMobile) {
                tableConfig.drawCallback = function(settings) {
                    ...
                    if (data.length > 0) {
                        var html = '';
                        data.each(function(bangleSize) {
                            html += createBangleSizeCard(bangleSize);
                        });
                        $('#mobile-bangle-sizes-container').html(html);
                    } else {
                        $('#mobile-bangle-sizes-container').html('<div class="text-center py-4"><p>No bangle sizes found</p></div>');
                    }
                ...
                tableConfig.initComplete = function(settings, json) {
                    ...
                    if (json && json.data && json.data.length > 0) {
                        var html = '';
                        json.data.forEach(function(bangleSize) {
                            html += createBangleSizeCard(bangleSize);
                        });
                        $('#mobile-bangle-sizes-container').html(html);
            ...
            table = $('#bangle-size-table').DataTable(tableConfig);
            isInitialized = true;
        ...
        function createBangleSizeCard(bangleSize) {
            return `
                <div class="bangle-size-card">
                    <div class="bangle-size-card-header">
                        <div class="bangle-size-card-title">Size: ${bangleSize.size}</div>
                    </div>
                    <div class="bangle-size-card-actions">
                        ${bangleSize.action}
                    </div>
                </div>
            `;
